...
def plot_obs_smf(redshift, ax, dict_style={}):
    ...
    ax.errorbar(f['logMstar'][sel] + np.log10(factor_from_Kroupa_to_Chabrier), f['logPhi'][sel],
             ...
             linestyle='none', color=dict_style['color'], mfc=dict_style['color'], mew=2, mec=dict_style['color'], lw=2, capsize=4, capthick=2,  zorder=-1)#, label=r"$\rm Weibel+24$" if redshift==3 else None)
    sel = (f['zmed'] == redshift) & (f['logPhi_loerr']>1e10)
    ax.errorbar(f['logMstar'][sel] + np.log10(factor_from_Kroupa_to_Chabrier), f['logPhi'][sel],
             yerr=(0.3*np.ones(len(f['logMstar'][sel])), np.zeros(len(f['logMstar'][sel]))), marker='o', ms=dict_style['ms'],
             linestyle='none', color=dict_style['color'], mfc=dict_style['color'], mew=2, mec=dict_style['color'], lw=2, capsize=4, capthick=1,  zorder=-1, uplims=True)
    ...
    ax.errorbar(f['logMstar'][sel] + np.log10(factor_from_Kroupa_to_Chabrier), np.log10(f['Phi']*1e-4)[sel],
            ...
            marker='o', ms=dict_style['ms'], linestyle='none', color=dict_style['color'], mfc=dict_style['color'], mew=2, mec=dict_style['color'], lw=2, capsize=4, capthick=2, zorder=-1)#, label=r"$\rm Harvey+25$" if redshift==3 else None)
    sel = (f['zmin'] == redshift - 0.5) & (f['Phi']-f['Phi_loerr']==0)
    ax.errorbar(f['logMstar'][sel] + np.log10(factor_from_Kroupa_to_Chabrier), np.log10(f['Phi']*1e-4)[sel],
            yerr=( 0.3 * np.ones(len(f['logMstar'][sel])), np.log10(f['Phi']+f['Phi_uperr'])[sel] - np.log10(f['Phi'][sel])),
            marker='o', ms=dict_style['ms'], linestyle='none', color=dict_style['color'], mfc=dict_style['color'], mew=2, mec=dict_style['color'], lw=2, capsize=4, capthick=1, zorder=-1, uplims=True)

    f = np.genfromtxt("./obdata/SMF/Wang2025.dat", names=True)
    sel = f['z'] == redshift
    ax.errorbar(f['logMstar'][sel] + np.log10(factor_from_Kroupa_to_Chabrier), f['logPhi'][sel],
            yerr=( f['logPhi'][sel] - f['lo'][sel], f['hi'][sel] - f['logPhi'][sel]),
            marker='o', ms=dict_style['ms'], linestyle='none', color=dict_style['color'], mfc=dict_style['color'], mew=2, mec=dict_style['color'], lw=2, capsize=4, capthick=2, zorder=-1)#, label=r"$\rm Wang+25$" if redshift==3 else None)
